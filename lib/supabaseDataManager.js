...
export const supabaseJobOps = {
  ...
  async autoSeed(table, seedCsvData) {
    ...
    if (result.data && result.data.length > 0) {
      // Map CSV columns to exact database column names
      const jobsToInsert = result.data.map((job) => {
        // Convert all keys to lowercase snake_case
        const mapped = {};
      ...
      console.log("Attempting to seed:", jobsToInsert.length, "jobs");
      console.log("Sample job:", JSON.stringify(jobsToInsert[0], null, 2));
